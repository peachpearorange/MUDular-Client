...
use crate::ui::editor::{EditorAction, ScriptEditor};
use crate::ui::gauges::render_gauges;
use crate::ui::input::InputLine;
use crate::ui::layout::render_layout;
use crate::ui::profile_list::{self, ProfileAction, TemplateAction};
...
fn find_file_recursive(dir: &std::path::Path, filename: &str) -> Option<std::path::PathBuf> {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_file() && path.file_name().is_some_and(|n| n.to_string_lossy().eq_ignore_ascii_case(filename)) {
            return Some(path);
        }
        if path.is_dir() {
...
fn apply_appearance(ctx: &egui::Context, appearance: &Appearance) {
    ...
    style.text_styles.insert(egui::TextStyle::Heading, egui::FontId::monospace(appearance.font_size + 2.0));
    ...
    if let Some([r, g, b]) = appearance.bg_color {
        ...
        style.visuals.faint_bg_color = egui::Color32::from_rgba_premultiplied(
            r.saturating_add(10), g.saturating_add(10), b.saturating_add(10), 255,
        );
    }
...
impl MudApp {
    ...
    fn connect_to_profile(&mut self, idx: usize) {
        ...
        info!("Script loaded: {} triggers, {} aliases, {} timers",
            engine.trigger_count(), engine.alias_count(), engine.timer_count());
    ...
    fn render_session_content(&mut self, ui: &mut egui::Ui, session_idx: usize) {
        ...
        let (gauges, status_text) = {
            let st = session.script_engine.state.lock().unwrap();
            (st.gauges.clone(), st.status_text.clone())
        };
        ...
        let status_height = if status_text.is_empty() { 0.0 } else { line_height + spacing };
        let bottom_height = line_height + gauge_height + status_height + separator_height + spacing;
        let pane_height = (available - bottom_height).max(100.0);
        trace!("Layout: available={available} bottom={bottom_height} pane_height={pane_height}");
...
impl eframe::App for MudApp {
    ...
    fn logic(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let scroll_line_height = self.active_tab.checked_sub(1)
            .and_then(|i| self.sessions.get(i))
            .map(|s| {
                    ...
                    for session in &mut self.sessions {
                        if session.profile_idx == profile_idx {
                            if let Err(e) = session.script_engine.load_script(&code) {
                                session.script_engine.append_system_message(&format!("[Script reload error: {e}]"));
                            } else {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _frame: &mut eframe::Frame) {
        ...
        ui.horizontal(|ui| {
            if self.active_tab == 0 {
                ui.add(egui::Label::new(egui::RichText::new(" Menu ").strong()).selectable(false));
            ...
            for (i, session) in self.sessions.iter().enumerate() {
                if self.active_tab == i + 1 {
                    ui.add(egui::Label::new(egui::RichText::new(format!(" {} ", session.name)).strong()).selectable(false));
        ...
        if self.active_tab == 0 {
            ...
            if self.show_template_picker {
                let action = profile_list::render_template_picker(
                    ui.ctx(), &self.templates, &mut self.show_template_picker,
                );
                match action {
