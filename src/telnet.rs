...
impl TelnetParser {
  pub fn new() -> Self { Self { state: State::Data } }

  pub fn reset(&mut self) { self.state = State::Data; }
  ...
  pub fn build_will(opt: u8) -> Vec<u8> { vec![IAC, WILL, opt] }

  pub fn build_wont(opt: u8) -> Vec<u8> { vec![IAC, WONT, opt] }

  pub fn build_do(opt: u8) -> Vec<u8> { vec![IAC, DO, opt] }

  pub fn build_dont(opt: u8) -> Vec<u8> { vec![IAC, DONT, opt] }
