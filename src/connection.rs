...
use crate::{protocol::{gmcp, mccp::MccpDecompressor, mssp},
            telnet::*};
...
impl Connection {
  ...
  pub fn disconnect(&self) { let _ = self.cmd_tx.send(ConnCommand::Disconnect); }
  ...
  loop {
    match reader.read(&mut buf).await {
      ...
      Ok(n) => {
        ...
        let data = if mccp.is_active() {
        ...
        } else if let Some(marker_pos) = find_mccp2_marker(raw) {
          let events = parser.parse(&raw[..marker_pos]);
          if !handle_telnet_events(
            events,
            &mut parser,
            &mut mccp,
            &mut mccp_pending,
          ...
          mccp_pending = false;
          parser.reset();
          let compressed_start = if raw[marker_pos + 3..].starts_with(&[IAC, SE]) {
            marker_pos + 5
          } else {
            marker_pos + 3
          };
          match mccp.decompress_data(&raw[compressed_start..]) {
            Ok(d) => d,
            Err(e) => {
              let _ = event_tx.send(ConnEvent::Error(e));
              break;
...
fn find_mccp2_marker(data: &[u8]) -> Option<usize> {
  data.windows(3).position(|window| window == [IAC, SB, OPT_MCCP2])
}

async fn handle_telnet_events(
  events: Vec<TelnetEvent>,
  parser: &mut TelnetParser,
  mccp: &mut MccpDecompressor,
  mccp_pending: &mut bool,
  gmcp_enabled: &mut bool,
  ...
  for event in events {
    match event {
      ...
      TelnetEvent::Negotiate(cmd, opt) => {
        ...
        let response = handle_negotiation(cmd, opt, mccp_pending, gmcp_enabled);
  ...
  match cmd {
    WILL => match opt {
      ...
      OPT_MSDP => {
        responses.push(TelnetParser::build_do(opt));
        let list_query =
          crate::protocol::msdp::encode_msdp_var("LIST", "REPORTABLE_VARIABLES");
